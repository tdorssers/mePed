...
#endif
#if defined(COMPILE_DECODE_RC5) || defined(COMPILE_DECODE_RC6)
static int getRClevel(decode_results *results, int *offset, int *used, int t1);
#endif
#ifdef COMPILE_DECODE_RC5
...
{
  // set pin modes
  DDRB &= ~_BV(PB4); // IR detector pin
  PORTB |= _BV(PB4); // pull-up
  DDRB |= _BV(PB5);  // indicator LED
  ...
  TCNT2 = (uint8_t) (INIT_TIMER_COUNT);  // timer reset
